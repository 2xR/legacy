...
from kivy.graphics.opengl import (glBlendFunc, GL_SRC_ALPHA, GL_ONE, GL_ZERO, GL_SRC_COLOR,
...
__all__ = ['ParticleSystem', 'Particle', 'EMITTER_TYPE_GRAVITY', 'EMITTER_TYPE_RADIAL']
...
class Particle(object):
    ...
    emit_radius = 0.0
    emit_radius_delta = 0.0
    emit_rotation = 0.0
    emit_rotation_delta = 0.0
...
class ParticleSystem(Widget):
    ...
    def _parse_color(self, name):
        return [float(self._parse_data(name, 'red')),
    ...
    def resume(self):
        ...
        Clock.schedule_interval(self._update, self.update_interval)
    ...
    def _advance_particle(self, particle, passed_time):
        ...
        if self.emitter_type == EMITTER_TYPE_RADIAL:
            ...
            particle.x = self.emitter_x - cos(particle.emit_rotation) * particle.emit_radius
            particle.y = self.emitter_y - sin(particle.emit_rotation) * particle.emit_radius
            ...
            distance_scalar = sqrt(distance_x * distance_x + distance_y * distance_y)
        ...
        particle.color = [particle.color[i] + particle.color_delta[i] * passed_time
                          for i in range(4)]

    def _raise_capacity(self, by_amount):
        old_capacity = self.capacity
        new_capacity = min(self.max_capacity, int(old_capacity + by_amount))
        self.particles.extend(Particle() for _ in xrange(new_capacity - old_capacity))
        self.num_particles = new_capacity
    ...
    def _render(self):
        ...
        for i in xrange(self.num_particles):
            ...
            if particle not in self.particles_dict:
                ...
                with self.canvas:
                    gl_color = Color(color[0], color[1], color[2], color[3])
                    self.particles_dict[particle]['color'] = gl_color
                    PushMatrix()
                    ...
                    self.particles_dict[particle]['rect'] = Quad(texture=self.texture,
                                                                 points=quad_points)
                ...
                self.particles_dict[particle]['rect'].points = quad_points
