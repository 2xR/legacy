...
class SPECj2004Sim(Simulator):
    ...
    def build(self):
        ...
        tree["SUT.LB+AS"] = CPUBalancer()
        for x in xrange(2):
            tree["SUT.LB+AS"].add(VirtualCPU(x, pCPU=tree["SUT.pCPU"]))
        tree["SUT.DB"] = Process()
    ...
    def compute_results(self):
        ...
        utz.LB_AS   = sum([vCPU.pCPU_utz.wmean() for vCPU in self.tree["SUT.LB+AS"]]) * 100.0
...
from params import mix0, servtime_2vm
sim = SPECj2004Sim("sim", config=Namespace(service_time=servtime_2vm, 
